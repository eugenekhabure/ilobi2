...
class PreRegistersReportController extends BackendController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($_POST) {
            ...
            $query = PreRegister::orderBy('id', 'DESC');
            if (auth()->user()->id != 1) {
                $query->where('added_by', auth()->user()->id);
            }

            if (!blank($dateBetween)) {
                $this->data['preRegisters'] = $query->whereBetween('created_at', [$dateBetween['from_date'], $dateBetween['to_date']])->get();
            } else {
                $this->data['preRegisters'] = $query->get();
            }
        }
