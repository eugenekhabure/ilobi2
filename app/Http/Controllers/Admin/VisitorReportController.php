...
class VisitorReportController extends BackendController
{
    ...
    public function index(Request $request)
    {
        ...
        // Handle form submission for filtering by date range
        if ($_POST) {
            $request->validate([
            ...
            $query = VisitingDetails::orderBy('id', 'DESC');
            if (auth()->user()->id != 1) {
                $query->where('added_by', auth()->user()->id);
            }

            if (!blank($dateBetween)) {
                $this->data['visitors'] = $query->whereBetween('created_at', [$dateBetween['from_date'], $dateBetween['to_date']])->get();
            } else {
                $this->data['visitors'] = $query->get();
            }
