...
class PreRegisterService
{
    ...
    public function all()
    {
        ...
        if ($user->id == 1) {
            ...
            if (auth()->user()->getrole->name == 'Employee') {
                return PreRegister::where('added_by', $user->id)->where(['employee_id' => auth()->user()->employee->id])->orderBy('id', 'desc')->get();
            } else {
                return PreRegister::where('added_by', $user->id)->orderBy('id', 'desc')->get();
            }
        }
    }
