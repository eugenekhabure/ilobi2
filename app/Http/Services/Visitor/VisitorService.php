...
class VisitorService
{
    ...
    public function all()
    {
        ...
        if ($user->id == 1) {
            ...
            if (auth()->user()->getrole->name == 'Employee') {
                return VisitingDetails::with('visitor','employee')->where('added_by', $user->id)->where(['employee_id' => auth()->user()->employee->id])->orderBy('id', 'desc')->get();
            } else {
                return VisitingDetails::with('visitor','employee')->where('added_by', $user->id)->orderBy('id', 'desc')->get();
            }
        }
    }
    ...
    public function make($request)
    {
        ...
        $input['editor_id']                  = 1;
        $input['reg_car_no']                 = $request->input('reg_car_no'); ##
